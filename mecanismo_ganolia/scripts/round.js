                ...
                for (let i = 0; i < resultado.categoria.length; i++) {
                    ...
                    if (resultado.categoria[i] == 'Moeda') {
                        ...
                        let novoBotaoCombinar = $('<button>', {
                            ...
                            style: 'margin-right: 50px;',
                            click: function() {
                                // LÃ³gica a ser executada quando o botÃ£o Ã© clicado
                            }
                        });
...
function exibicao(um,dois,tres,quatro,cinco,imagens) {
    ...
    if (!Array.isArray(imagens) || imagens.length === 0) {
        // Se nÃ£o houver imagens, esconde o elemento principal
        $(um).hide();
        $(dois).hide();
        $(tres).hide();
        $(quatro).hide();
    ...
    $.ajax({
        ...
        success: function (resultado) { 
            if (resultado.success) {
                ...
                $('#qtdAtaque').html("Ataques disponÃ­veis:" + resultado.quantidade).show();
